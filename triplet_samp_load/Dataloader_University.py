...
augmenter = iaa.Sequential([
    iaa.SomeOf((1, 3), [
        ...
        iaa.Add((10, 50), per_channel=0.5),  # 增加亮度
        iaa.Multiply((0.5, 1.5), per_channel=0.5),  # 调整亮度
        iaa.LinearContrast((0.75, 1.5)),  # 对比度
        iaa.Snowflakes(flake_size=(0.2, 0.7), speed=(0.01, 0.05)),  # 雪花
        iaa.Rain(speed=(0.1, 0.3)),  # 下雨
        iaa.Fog(),  # 雾
    ], random_order=True)
])
...
class Dataloader_University(Dataset):
    def __init__(self, root, transforms, names=['satellite', 'drone'], add_weather=False):
...
if __name__ == '__main__':
    ...
    datasets = Dataloader_University(root="/home/oem/桌面/drone/TriSSA/data/University-1652/train", transforms=transform_train_list, names=['satellite', 'drone'])
    samper = Sampler_University(datasets, 16)

    dataloader = DataLoader(datasets, batch_size=16, num_workers=0, sampler=samper, collate_fn=train_collate_fn)
    i = 0
    for data_s, data_d in dataloader:
        i += 1
        inputs2, labels2, label_num = data_s
