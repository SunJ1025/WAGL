...
class Dino(nn.Module):
    def __init__(self, type):
        super().__init__()
        # get feature model
        model = torch.hub.load("facebookresearch/dinov2", type, pretrained=True).to(device)
        ...
        self.classifier = LinearClassifier(
            1536, use_n_blocks=1, use_avgpool=True, num_classes=100
        ).to(device)
...
#         print(
#             '{} Loss: {:.4f} Acc: {:.4f} Time: {:.0f}m {:.0f}s'.format(phase, epoch_loss, epoch_acc, time_elapsed // 60,
#                                                                        time_elapsed % 60))  # Modify this line
...
if __name__ == '__main__':
    dino = Dino('dinov2_vitb14')
    sample_input = torch.randn(1, 3, 224, 224).to(device)
    out = dino(sample_input)
    # print(out.shape)
    # print(out)